import { useEffect } from 'react'
import UserEoiSection from './manager/UserEoiSection';
import DemandsSection from './Admin/DemandsSection';
import { formatDateOnly } from './manager/utils'

export default function AdminDashboard({ allData, adminPage }) {
  const eoiRows = Array.isArray(allData?.eoiData) ? allData.eoiData : [];
  const demandRows = Array.isArray(allData?.demandNotes) ? allData.demandNotes : [];
  
  useEffect(() => {
    const hasData = eoiRows.length > 0;
    if (!hasData) return;
    console.log("AdminDashboard allData:", allData);
  }, [allData, eoiRows.length]);

  return (
    <div className="admin-dashboard-layout flex flex-col gap-6">
      <div className="admin-dashboard-content min-w-0">
{adminPage === "demand-notes" && (
  <DemandsSection demandRows={demandRows}  formatDateOnly={formatDateOnly} />
)}

        {adminPage === "eoi-data" && (
          <UserEoiSection
            eoiRows={eoiRows}
            formatDateOnly={formatDateOnly}
          />
        )}
      </div>
    </div>
  );
}